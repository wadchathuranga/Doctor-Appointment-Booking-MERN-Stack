...
export const getDoctorProfile = async (req, res) => {
  ...
  try {
    ...
    if (!doctor) {
      return res
        .status(404)
        .Json({ success: false, nessage: "Doctor not found!" });
    }

    const { password, ...rest } = doctor._doc;
    const appointments = await Booking.find({ doctors: doctorId });
    ...
    res
      .status(500)
      .json({ success: false, message: "Sonething went wrong, cannot get" });
  }
};
